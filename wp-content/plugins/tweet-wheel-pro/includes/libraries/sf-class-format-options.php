...
?>
			...
			<tr valign="top">
				<th scope="row"<?php echo $grouped; ?>>
					...
					<?php if ( !$grouped ) : ?>
						<label for="<?php echo $name; ?>" class="description"><?php _e( $title, TWP_TEXTDOMAIN ); ?></label>
					<?php endif; ?>
				...
				if ( $type != $header ) continue; ?>
					<tr>
						<th scope="col" colspan="2">
							<h3 class="title"><?php _e( $title, TWP_TEXTDOMAIN ); ?></h3>
							<?php _e( $description, TWP_TEXTDOMAIN ); ?>
						</th>
					</tr>
		<?php endforeach; ?>
...
?>
			<input name="<?php echo $name; ?>"
				 ...
				 style="<?php echo $css; ?>"
				 placeholder="<?php _e( $placeholder, TWP_TEXTDOMAIN ); ?>"
		...
		case 'checkbox':
			...
			if ( $multiple ) :
				...
				foreach ( $options as $key => $desc ) : ?>
						...
						<label <?php echo $value[$key] == 1 ? 'class="active"' : '' ?> for="<?php echo $id . '_' . $key; ?>">
		...
		case 'radio':
			...
			foreach ( $options as $key => $val ) : ?>
						...
						/>
						<?php _e( $val, TWP_TEXTDOMAIN ); ?>
						</label><br />
			<?php endforeach;
			_e( $description, TWP_TEXTDOMAIN );
			break;
		...
		case 'single_select_page':
			...
			if ( $select2 ) : ?>
				<script type="text/javascript">jQuery(function() {jQuery("#<?php echo $id; ?>").select2({ allowClear: true, placeholder: "<?php _e( 'Select a page...', TWP_TEXTDOMAIN ); ?>", width: '350px' });});</script>
			<?php endif;
		...
		case 'select':
			...
			<?php foreach ( $options as $key => $val ) : ?>
						<option value="<?php echo $key; ?>" <?php self::selected( $selected, $key ); ?>><?php _e( $val, TWP_TEXTDOMAIN ); ?></option>
			<?php endforeach; ?>
			</select>
		...
		{
			return $tip ? sprintf( '<a href="#" title="%s" class="sf-tips" tabindex="99"></a>', __( $tip, TWP_TEXTDOMAIN ) ) : '';
		}
